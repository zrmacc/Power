...
&= \sum_{i=1}^{n} \big\{ Y_{i}\cdot (W_{i}'\beta) - \ln(1 + e^{W_{i}'\beta}) \big\}.
\end{align*}
...
\begin{align*}
T_{W} = n \cdot \hat{\beta}_{X}'\big(i_{\beta_{X}\beta_{X}'} - i_{\beta_{X}\gamma'} i_{\gamma\gamma'}^{-1}i_{\gamma\beta_{X}'}\big)\hat{\beta}_{X}.
\end{align*}
